...
inline std::vector<std::vector<char>> stringTo2dCharVector(std::string input)
{
...
inline double stringToDouble(std::string input)
{
    return stod(input);
}
...
{
public:
    std::unordered_map<std::string, std::unique_ptr<MemberFunctionBase<T>>>
        funcMap;
...
public:
    Excecutor(std::string path)
        : instance(nullptr)
    {
        parsefromInputs(path);
    };
    ~Excecutor()
    {
        if (instance != nullptr) {
...
#define REGISTER(Class, func) exc.registerMemberFunction(#func, &Class::func);